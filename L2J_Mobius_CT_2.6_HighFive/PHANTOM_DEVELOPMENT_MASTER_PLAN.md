...
- typed `RouteAttempt` сохраняет Navigation terminal status через Goal 017 в Rift; terminal no-route не создаёт route/deadline ownership, а Rift возвращается к обычному replan без same-pulse resend.
...
Original-plan numbering drifted: фактический Goal028 был использован для operator controls/diagnostics/replay. Siege scope теперь закрыт bounded Goal035 со статусом `SUCCESS`: один Giran vertical через native `Castle`/`Siege`/`SiegeScheduleData`, native registration/sides, shared Navigation/Combat, deterministic roles, retreat/recovery и guarded test fixture. Это не universal multi-castle siege AI.
...
Original-plan universal scope остаётся `DEFERRED_NOT_IMPLEMENTED`. Фактический Goal029 был использован для scale/soak; bounded representative slice закрыт Goal036 как `SUCCESS`: Q102/Q152, Q401 Fighter→Warrior, Kamaloka 57 и Pailaka 128 через native owners, без universal solver.
...
2. **Goal035 — siege gameplay slice — `SUCCESS`.** Bounded Giran castleId=3 vertical использует native schedule/registration/sides/zones/doors, existing Navigation/Combat/Party support owners, deterministic capped roles и retreat/recovery/restart cleanup. Focused `8/8`, affected regression routes `6/6`, fresh full verify и standalone jar PASS на guarded test DB; production DB unused.
3. **Goal036 — bounded quests/instances slice — `SUCCESS`.** Строгий versioned whitelist поддерживает Q102/Q152 от native start до completion с accepted acquisition subset, Q401 и Fighter→Warrior только через normal-player owner `ElfHumanFighterChange1`, Kamaloka template 57 и Pailaka 128 через native Quest/Instance lifecycle. Focused `8/8`, affected regressions, fresh full verify и standalone jar PASS на guarded test DB; это не universal solver.
4. **Goal037 — full High Five quest-script inventory + rates normalization/parity — `SUCCESS`.** Deterministic JDK AST inventory охватывает 543/543 Java source и 7,066/7,066 rate-relevant sites в `dist/game/data/scripts/quests/**`; unknown/stale/new source/site fail closed. Исправлены 1,876 доказанных bypass/mismatch в 331 source: 1,503 control/key/singleton grants, 369 terminal item rewards и четыре cap-aware objective grants Q102/Q152/Q401; 1,538 fixed exceptions явно source-hashed, raw chance не умножается quest-item amount rate. Production corpus compile/load, distinct 1x/non-1x Player и Phantom ACTIVE/BACKGROUND, Q401/Pailaka controls, XP/SP/drop/spoil/manor, affected gates, fresh verify и standalone jar PASS только на guarded test DB; universal quest solver не заявлен.
...
6. **Goal039 — final full-vision release gate + freeze — `ACCEPT`.** Resume 9 исправил только historical Goal016 byte-source owner: accepted/descendant verification читает exact manifest/payload bytes из unique completion commit, а implementation-HEAD working mode сохранён. Один fresh full `ant verify`, standalone final jar и fresh Goal034 real stack прошли; declared-scope matrix `28/28`, static `8/8`, documentation `2/2`. Roadmap v5 **FINISHED**: `FEATURE_COMPLETE_FOR_DECLARED_SCOPE`.

Goal039 принят; статус зафиксирован как `FEATURE_COMPLETE_FOR_DECLARED_SCOPE`.
