...
foreach($input in $manifest.source_hashes){$p=Join-Path $DataRoot ($input.path.Substring(5).Replace('/',[IO.Path]::DirectorySeparatorChar));Assert ((Hash $p) -eq $input.sha256) "source drift: $($input.path)"}
