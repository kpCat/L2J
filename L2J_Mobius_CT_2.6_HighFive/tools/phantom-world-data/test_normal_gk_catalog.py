...
class NormalGatekeeperCatalogTest(unittest.TestCase):
    ...
    def test_only_proven_normal_compound_legs_are_emitted(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(40, len(legs))
            self.assertEqual(40, len({leg["id"] for leg in legs}))
            self.assertTrue({"30177", "30848", "31275"}.issubset({leg["teleporterNpcId"] for leg in legs}))
