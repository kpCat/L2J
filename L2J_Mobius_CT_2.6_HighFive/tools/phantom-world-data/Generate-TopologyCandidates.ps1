...
$usedCore=@{};$spatial=New-Object 'System.Collections.Generic.List[object]';$topology=New-Object 'System.Collections.Generic.List[object]';$nodeIds=@{};$anchorIds=@{};$points=New-Object 'System.Collections.Generic.List[object]';$ambiguous=0;$exact=0;$needsAnchor=0
foreach($r in $ordinary) {
    ...
    $points.Add([pscustomobject]@{key=$r.coverage_key;node=$nodeId;x=$x;y=$y;cell=$cell;region=$locId;instance=$r.instance_id;refs=$r.source_path})
}
...
$counts=[ordered]@{ordinary_registry_rows=$ordinary.Count;exact_core_farming_rows=$exact;generated_farming_rows=($ordinary.Count-$exact);needs_anchor_geodata=$needsAnchor;blocked_ambiguous=$ambiguous;unmatched_existing_core_farming=($coreFarms.Count-$usedCore.Count);duplicate_node_ids=0;duplicate_anchor_ids=0;duplicate_route_ids=0;route_candidates=$routes.Count;local_walk=@($routes | Where-Object candidate_mode -eq 'LOCAL_WALK').Count;region_link=@($routes | Where-Object candidate_mode -eq 'REGION_LINK').Count;teleport_fact=@($routes | Where-Object candidate_mode -eq 'TELEPORT_FACT').Count;existing_core_edges=@($core.SelectNodes('/topology/edge')).Count}
