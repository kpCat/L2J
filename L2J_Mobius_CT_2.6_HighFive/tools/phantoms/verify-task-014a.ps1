...
$requiredParent = "696689987276137f6a7f3661329171c9ee65e6f9"
$requiredBranch = "feature/phantom-world"
$requiredSubject = "fix(phantoms): harden commerce ownership and integration"
...
if ($head -ne $requiredParent)
{
	...
	Assert-True ($parents.Count -eq 1) "Goal 014A HEAD is a merge commit."
	Assert-True ($parents[0] -eq $requiredParent) "Goal 014A HEAD is not a direct child of the required parent."
	Assert-True ((& git -C $repositoryRoot show -s --format=%s $head).Trim() -eq $requiredSubject) "Goal 014A commit subject is not exact."
	Assert-True ([int](& git -C $repositoryRoot rev-list --count "$requiredParent..$head") -eq 1) "Goal 014A graph contains more than one child commit."
...
foreach ($fact in @("commerce-hardening", "new L2jCommerceBackend(", "new Merchant(", "new Teleporter(", "materialization.dematerialize(", "materialization.materialize(", "player.storeMe()", "currentPersistenceClaims()", "GOAL_REVISION_CONFLICT", "STALE_GOAL_REVISION", "TELEPORT_PENDING"))
{
