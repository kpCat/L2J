...
$acceptedCommit = "a546dae868d93d54ec4bc6e1836080b90f810167"
...
function Read-PinnedBlobBytes([string] $local)
{
	$object = "${acceptedCommit}:$moduleRelative/$local"
	...
	Assert-True ($process.ExitCode -eq 0) "Cannot read accepted Goal 015 blob $local`: $errorText"
	return ,$bytes.ToArray()
}

function Read-PinnedUtf8([string] $local)
{
	$encoding = [Text.UTF8Encoding]::new($false, $true)
	return $encoding.GetString((Read-PinnedBlobBytes $local))
}

function Get-PinnedSha256([string] $local)
{
	$sha256 = [Security.Cryptography.SHA256]::Create()
	try
	{
		return ([BitConverter]::ToString($sha256.ComputeHash((Read-PinnedBlobBytes $local)))).Replace("-", "")
	}
	finally
	{
		$sha256.Dispose()
...
function Assert-PinnedFile([string] $local)
{
	& git -C $repositoryRoot cat-file -e "${acceptedCommit}:$moduleRelative/$local"
	Assert-True ($LASTEXITCODE -eq 0) "Missing accepted Goal 015 file: $local"
}
...
Assert-True ((& git -C $repositoryRoot show -s --format=%P $acceptedCommit).Trim() -eq $requiredParent) "Accepted final Goal 015 commit has the wrong parent."
Assert-True ((& git -C $repositoryRoot show -s --format=%s $acceptedCommit).Trim() -eq $requiredSubject) "Accepted final Goal 015 commit subject changed."

& git -C $repositoryRoot merge-base --is-ancestor $acceptedCommit $head
Assert-True ($LASTEXITCODE -eq 0) "Accepted final Goal 015 commit is not an ancestor of HEAD."
Assert-True ([int](& git -C $repositoryRoot rev-list --count "$requiredParent..$acceptedCommit") -eq 1) "Accepted Goal 015 graph has more than one child commit."
$graphMode = "historical-accepted"

$changedPaths = @(& git -c core.autocrlf=false -C $repositoryRoot diff --name-only "$requiredParent..$acceptedCommit")
...
{
	$actual = Get-PinnedSha256 $entry.Key
	Assert-True ($actual -eq $entry.Value) "Task package payload changed: $($entry.Key)"
}

$stateText = Read-PinnedUtf8 "java/org/l2jmobius/gameserver/phantoms/background/PhantomBackgroundState.java"
...
$modelText = Read-PinnedUtf8 "java/org/l2jmobius/gameserver/phantoms/background/PhantomBackgroundModel.java"
...
$authorityText = Read-PinnedUtf8 "java/org/l2jmobius/gameserver/phantoms/background/L2jPhantomBackgroundAuthority.java"
...
$topologyText = Read-PinnedUtf8 "dist/game/data/phantoms/topology/high-five-core.xml"
...
$topologyDiff = @(& git -c core.autocrlf=false -C $repositoryRoot diff --unified=0 "$requiredParent..$acceptedCommit" -- "$moduleRelative/dist/game/data/phantoms/topology/high-five-core.xml")
...
$transactionText = Read-PinnedUtf8 "java/org/l2jmobius/gameserver/phantoms/background/PhantomBackgroundTransaction.java"
...
$serviceText = Read-PinnedUtf8 "java/org/l2jmobius/gameserver/phantoms/background/PhantomBackgroundService.java"
...
$goalText = Read-PinnedUtf8 "java/org/l2jmobius/gameserver/phantoms/background/PhantomBackgroundGoalSpec.java"
...
$identityText = Read-PinnedUtf8 "java/org/l2jmobius/gameserver/phantoms/player/PhantomIdentityLeaseRegistry.java"
$clientText = Read-PinnedUtf8 "java/org/l2jmobius/gameserver/network/GameClient.java"
$loginGuardText = Read-PinnedUtf8 "java/org/l2jmobius/gameserver/phantoms/background/PhantomBackgroundLoginGuard.java"
$materializationText = Read-PinnedUtf8 "java/org/l2jmobius/gameserver/phantoms/player/PhantomMaterializationService.java"
...
$planningText = Read-PinnedUtf8 "java/org/l2jmobius/gameserver/phantoms/decision/PhantomPlanningContext.java"
$stepText = Read-PinnedUtf8 "java/org/l2jmobius/gameserver/phantoms/decision/PhantomStepContext.java"
$engineText = Read-PinnedUtf8 "java/org/l2jmobius/gameserver/phantoms/decision/PhantomDecisionEngine.java"
...
$systemText = Read-PinnedUtf8 "java/org/l2jmobius/gameserver/phantoms/PhantomSystem.java"
...
$suiteText = Read-PinnedUtf8 "test/java/org/l2jmobius/tests/phantoms/PhantomBackgroundSuite.java"
...
$buildText = Read-PinnedUtf8 "build.xml"
$launcherText = Read-PinnedUtf8 "test/java/org/l2jmobius/tests/phantoms/PhantomTestLauncher.java"
...
$productionFiles = @(& git -C $repositoryRoot ls-tree -r --name-only $acceptedCommit -- "$moduleRelative/java/org/l2jmobius/gameserver/phantoms/background")
$productionText = ($productionFiles | ForEach-Object { Read-PinnedUtf8 $_.Substring($moduleRelative.Length + 1) }) -join "`n"
...
$architectureText = Read-PinnedUtf8 "docs/phantoms/architecture/BACKGROUND_FARMING_RECONCILIATION_CONTRACT.md"
...
$roadmapText = Read-PinnedUtf8 "docs/PHANTOM_BOTS_ROADMAP.md"
...
$masterPlanText = Read-PinnedUtf8 "PHANTOM_DEVELOPMENT_MASTER_PLAN.md"
...
$reviewText = Read-PinnedUtf8 "docs/phantoms/reviews/015-production-loot-disposition-unblock-review.md"
...
$positionReviewText = Read-PinnedUtf8 "docs/phantoms/reviews/015-background-position-canonicalization-review.md"
...
$anchorReviewText = Read-PinnedUtf8 "docs/phantoms/reviews/015-background-anchor-tolerance-review.md"
...
$reportText = Read-PinnedUtf8 "docs/phantoms/reports/015-background-farming-reconciliation.md"
Assert-True (($reportText -split "\r?\n").Count -le 170) "Goal 015 report exceeds the 170-line efficiency bound."
...
$finalReviewPath = Join-Path $moduleRoot "docs/phantoms/reviews/015-background-farming-final-review.md"
Assert-True (Test-Path -LiteralPath $finalReviewPath -PathType Leaf) "Final independent Goal 015 review is missing."
$finalReviewText = Read-Utf8Strict $finalReviewPath
Assert-True ($finalReviewText.Contains('`ACCEPT`') -and $finalReviewText.Contains($acceptedCommit) -and $finalReviewText.Contains("TASK015_VERIFIER_OK")) "Final independent Goal 015 review truth is incomplete."
...
foreach ($path in $changedPaths)
{
	$local = $path.Substring($moduleRelative.Length + 1)
	if ($textExtensions -contains [IO.Path]::GetExtension($local))
	{
		$text = Read-PinnedUtf8 $local
...
Write-Output "acceptedCommit=$acceptedCommit"
