...
Целевые веса: `DAMAGE=55`, `TANK=8`, `HEALER=8`, `ENHANCEMENT=12`, `CONTROL=7`, `ECONOMY=10`; их сумма обязана быть ровно `100`. Сначала по этим весам выбирается архетип, а затем уже внутри архетипа выбирается совместимая классовая lineage. Архетип — это намерение развития и не обозначает третью профессию: `ENHANCEMENT=12` не означает, что 12% населения станет Prophet. DD намеренно преобладают, а персонажи поддержки остаются менее частыми, но гарантированно доступными через совместимые lineage. `ECONOMY` сохраняет Dwarven spoil/craft lineage. Профессии, class transfer и квесты продолжают выполняться штатными механизмами High Five.
