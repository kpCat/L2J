...
5. Goal030C1 — `IMPLEMENTED_PENDING_INDEPENDENT_REVIEW`: canonical clan/alliance/war context подключён к Clan/Party/PvP, а штатное изгнание создаёт точную личную betrayal-memory; native лидерство остаётся источником истины.
6. Goal030C2 — `NOT_STARTED`: directive social events и PK/karma recovery.
7. CP2 — `NOT_STARTED_AFTER_HUMANIZATION`: автономный cross-domain alpha-сценарий на едином мире; пакет должен быть сформирован заново после независимого принятия 030C1 и завершения 030C2.
8. CP3: финальное решение о выпуске, rollback и release-level restart/failure recovery.

Release sequence: `CP1 -> 030A -> 030B/030B1 -> 030C1 -> 030C2 -> CP2 -> CP3`. Coverage matrix и число её строк не меняются.
