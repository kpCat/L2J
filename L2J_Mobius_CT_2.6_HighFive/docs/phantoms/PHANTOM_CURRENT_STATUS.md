...
| **Goal032 Phantom-only reset/reseed** | **SUCCESS** | Goal032 ownership 3/3; reset/reseed 2/2; focused regressions | Shared state без безопасного owner блокирует reset | Следующий Goal033 ecology: повтор после Goal033A SUCCESS |
| **Goal033 living population ecology** | **READY_TO_REPEAT_AFTER_GOAL033A_SUCCESS** | Causal prerequisite закрыт Goal033A; Goal033 historical report сохранён | Ecology/schedules/pace всё ещё не реализованы; direct XP запрещён | Повторить Goal033, не Goal034 |
| **Goal033A causal historical Background catch-up** | **SUCCESS** | Real-data planner, lifecycle baseline, atomic minute cursor, restart/death semantics и fences; focused tests + final jar PASS | Не реализует Goal033 ecology | Повторить Goal033 |
| **Goal033A1 canonical population topology ingress** | **SUCCESS; topology blocker CLOSED BY Goal033A1** | 38/38 exact ingress; 7 real farms; 80/80 factual Background edges; Goal033A1 4/4; focused regressions PASS | Не реализует ecology; historical catch-up закрыт Goal033A | Повторить Goal033 |
...
| Goal033 — Living population ecology | READY_TO_REPEAT_AFTER_GOAL033A_SUCCESS | Causal catch-up prerequisite закрыт; повторить Goal033 без direct XP |
