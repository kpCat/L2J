...
| Combat | IMPLEMENTED_AND_RELEASE_COVERED | matrix `combat`; `phantom-combat-server-integration-test`; Goal035 explicit siege branch | Goal035 поддерживает только bounded Giran Player/door targets | Сохранять native combat owner |
...
| Goal036 — Bounded quests/instances slice | PLANNED; NOT_STARTED | Whitelist lifecycle, class transfer, supported Kamaloka/Pailaka; без universal solver |
...
После успешного Goal035 остаются три content/feature stage — Goal036–Goal038
...
Текущее следующее действие: Goal036 — следующий planned feature Goal, но он
остаётся `NOT_STARTED` и требует отдельной явной задачи. Goal035 не начинал
Goal036 и не менял Roadmap v5 scope Goal037–039.
