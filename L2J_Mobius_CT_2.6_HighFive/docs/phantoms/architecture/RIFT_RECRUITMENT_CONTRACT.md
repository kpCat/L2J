...
Goal 023C добавляет typed `RouteAttempt`: Navigation `ACCEPTED` без terminal result даёт `PENDING`, completed usable route — `READY`, completed без route (`NO_PATH`, `BACKEND_FAILURE` и другие terminal status) — `FAILED`, submission rejection — `REJECTED`/`UNAVAILABLE`. Terminal no-route не создаёт `_routeByGroup` или deadline; bounded terminal receipt сохраняет exact route identity/generation/destination и `PhantomNavigationResult.Status` только до `reconcileTerminalRoute`. `RouteActivity.NONE` означает действительное отсутствие ownership, а Rift на terminal failure выходит из `OBSERVE_ROUTE` к обычному replan без same-pulse resend.
