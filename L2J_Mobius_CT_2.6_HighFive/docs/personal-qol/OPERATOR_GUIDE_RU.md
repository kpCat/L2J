...
## Phantom Summoner и Servitor

QOL-009 не добавляет операторский переключатель: это внутренняя корректность уже включённого Phantom combat. Для summoner classes Phantom использует только принадлежащий Player live true Servitor; обычные Pet и BabyPet этим путём не управляются. При отсутствии Servitor backend выбирает только известный canonical summon skill из progression catalog и вызывает штатный `Player.useMagic`, поэтому Spirit Ore, MP, reuse, summon conditions, lifetime/upkeep и zone/instance rules остаются обязательными. Active Servitor skill берётся из live NPC parameters и исполняется через `Summon.useMagic`; при невозможности skill Servitor возвращается к штатной атаке.

При окончании или отмене боя Servitor прекращает принадлежащее Phantom действие и возвращается в native follow. В normal PvP Servitor вражеского summoner может быть временной tactical целью, но его owner Player остаётся canonical PvP participant и consequence context. Новых PvP/PK/karma mutations, телепорта или обхода geodata нет.
...
ant qol-008-verify
ant qol-summoner-servitor-combat-test
ant qol-009-affected-test
ant qol-009-freeze-test
