...
2. **Goal033 — Living population ecology — `BLOCKED_033_CAUSAL_CATCHUP_ENTRYPOINT_REQUIRED`.** Read-first audit подтвердил отсутствие rubber-band к уровню human player и пригодность Population/Profile/Social owners, но Background исполняет только заранее сформированный `farm.background` goal и не выбирает target/anchor. Нужен bounded prerequisite для canonical target/anchor planning, initial Background capture, exactly-once interval cursor и materialization fence; direct XP/free resources запрещены.
   - **Goal033A — Causal historical Background catch-up prerequisite — `BLOCKED_033A_CANONICAL_TOPOLOGY_INGRESS_REQUIRED`.** Canonical population creation positions не идентифицируют ни одного committed topology anchor, а shipped topology содержит только один `BACKGROUND` edge: `giran.route.north -> giran.farming.22859`. Existing `BackgroundService.afterStore()` поэтому не может создать initial baseline для новой managed population без direct teleport, выдуманного route или второго Player lifecycle. Production catch-up code не добавлен. Минимальный следующий unblock: отдельный bounded factual topology-ingress slice для canonical creation positions с production/geodata evidence и route tests; затем повторить Goal033A и только после его ACCEPT повторить Goal033.
     - **Goal033A1 — Canonical population topology ingress — `SUCCESS`.** Все 38/38 authoritative creation positions представлены unique exact ingress anchors; семь real-spawn low-level farming destinations связаны 80 factual one-way `BACKGROUND` edges, production Navigation/exact-anchor/travel/negative gates PASS. Topology corpus v3: 110 nodes / 110 anchors / 83 edges. Blocker Goal033A закрыт Goal033A1, historical Goal033A остаётся `BLOCKED`; следующий шаг — повторить Goal033A.
