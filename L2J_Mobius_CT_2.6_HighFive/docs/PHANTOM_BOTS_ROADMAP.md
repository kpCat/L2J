...
3. **Goal034 — automated black-box local stack acceptance — `BLOCKED`.** Canonical runtime layout закрыт: sandbox содержит полный `dist/game/data` snapshot, exact roots и pre-spawn gate; три full verify и три standalone jar green. Реальный LS/GS READY и registered, но LIVING остаётся `managed=10/online=2..3` при требуемом `10/5`; native restart/gen2 не достигнуты, лимит 3 full verify исчерпан.
