...
public final class PhantomRaidDecision
{
	...
	private final PhantomRaidAssemblyService _assembly;

	public PhantomRaidDecision(PhantomRaidAssemblyService assembly)
	{
		_assembly = assembly;
	}
	...
	public void registerHandlers(PhantomStepHandlerRegistry registry)
	{
		registry.register(PREPARE_ACTION, context ->
		{
			...
			final var result = _assembly.advance(context.profileId(), context.goal().goalId(), context.goal().revision());
			if (result.status() == AssemblyStatus.READY_AT_STAGING)
			{
				return PhantomStepResult.of(PhantomStepResult.Type.COMPLETE_GOAL, result.reasonKey());
			}
			if (result.status() == AssemblyStatus.CANCELLED)
			{
				return PhantomStepResult.of(PhantomStepResult.Type.CANCELLED, result.reasonKey());
			}
			if ((result.status() == AssemblyStatus.BLOCKED) || (result.status() == AssemblyStatus.EXPIRED))
			{
		...
		registry.register(PARTICIPATE_ACTION, context ->
		{
			...
			final ParticipationOutcome outcome = _assembly.participation(context.profileId(), context.goal().goalId(), context.goal().revision());
			return switch (outcome)
			{
				case WAITING -> PhantomStepResult.of(PhantomStepResult.Type.REPLAN, "raid.participate.waiting");
				case JOINED -> PhantomStepResult.of(PhantomStepResult.Type.COMPLETE_GOAL, "raid.participate.joined");
				case EXPIRED -> PhantomStepResult.of(PhantomStepResult.Type.FAIL_GOAL, "raid.participate.expired");
				case IMPOSSIBLE -> PhantomStepResult.of(PhantomStepResult.Type.FAIL_GOAL, "raid.participate.impossible");
			};
		});
	}
